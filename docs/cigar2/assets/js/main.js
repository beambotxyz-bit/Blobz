...
(function() {
	...
	class Cell {
		constructor(id, x, y, s, name, nameColor, cellColor, borderColor, skin, flags) {
			...
			this.s = s;
			this.ns = s;
			this.name = filterNicknameForClient(name);
		...
		update(relativeTime) {
			...
			const dt = Math.max(Math.min((relativeTime - this.updated) / settings.animationSpeed, 1), 0);
			let diedBy;
		...
		drawShape(ctx) {
			...
			if (settings.showTransparent) {
				ctx.globalAlpha = 1 - parseFloat(settings.transparentAlpha) || 1;
	...
	function filterNicknameForServer(name) {
		if (!name || name === EMPTY_NAME) {
		...
		if (filters && filters.initialized) {
			if (filters.hasProfanity(name, 'en')) {
				name = filters.filter_nickname(name, 'en');
			}

			if (filters.hasProfanity(name, 'tr')) {
				name = filters.filter_nickname(name, 'tr');
			}

			return filters.remove_spam(name);
		}
	...
	const SERVERS = ['agar' + '.' + LOCATION + '/ws2/', 'agar2' + '.' + LOCATION + '/ws2/', 'agar3' + '.' + LOCATION + '/ws2/'];
	...
	function wsInit(url) {
		...
		/*if (!url.startsWith('agar3')) {
			...
			clearTimeout(announcePresence1);
			clearTimeout(announcePresence2);
		}*/
	}
	...
	function wsMessage(data) {
		...
		switch (packetId) {
			...
			case 0x10: {
				...
				while (true) {
					...
					if (cells.byId.has(id)) {
						...
						if (name) cell.name = filterNicknameForClient(name);
			...
			case 0x31: {
				...
				for (let i = 0; i < count; ++i) {
					...
					leaderboard.items.push({
						me: isMe,
						name: filterNicknameForClient(lbName || EMPTY_NAME)
					});
				}
			...
			case 0x62: {
				...
				let name = rawName || EMPTY_NAME;
				let fp2 = null;

				// Extract fp2 from rawName format: <skin|nameColor|cellColor|borderColor||fp2>name
				if (rawName && rawName.indexOf('<') === 0) {
					const endIndex = rawName.indexOf('>');
					if (endIndex !== -1) {
						const skinPart = rawName.substring(1, endIndex);
						if (skinPart) {
							const parts = skinPart.split('|');
							if (parts.length > 5 && parts[5]) {
								fp2 = parts[5].trim() || null;
							}
						}
					}
				}

				// Apply nickname filtering if enabled (skip for server/admin/mod names)
				if (!flags.server && !flags.admin && !flags.mod) {
					name = filterNicknameForClient(name);
				}
				...
				// Apply word filtering if enabled (skip for server/admin/mod messages)
				let filteredMessage = message;
				if (settings.useWordFilters && filters && filters.initialized && !flags.server && !flags.admin && !flags.mod) {
					filteredMessage = filters.filter_text(filters.filter_text(message, 'en'), 'tr');
				}
	...
	let announcePresence1;
	let announcePresence2;
	let quadtree;
	let interval;
	let mcinterval;
	let filters = null;
	...
	const settings = {
		...
		// Button positions (stored as {left/right, top/bottom, value})
		buttonPositions: {},
		// Key bindings
		keyFeed: 'w',
		keySplit: ' ',
	...
	// Function to validate nickname and set custom validity
	function isNicknameAllowed(nickname, inputElement) {
	...
	function loadSettings() {
		...
		for (const prop in settings) {
			...
			if (elm) {
				...
				} else if (prop === 'buttonPositions') {
					if (Object.hasOwnProperty.call(obj, prop) && typeof obj[prop] === 'object' && obj[prop] !== null) {
						settings[prop] = obj[prop];
					} else {
						settings[prop] = {};
					}
	...
	function findFp2FromName(name) {
		...
		// Search through recent chat messages to find fp2 for this name
		for (let i = chat.messages.length - 1; i >= 0; i--) {
			const msg = chat.messages[i];
			if (msg.name && msg.fp2) {
				const msgCleanName = msg.name.replace(/^\[(?:ADMIN|MOD|SERVER)\]\s*/, '');
				if (msgCleanName === cleanName) {
					return msg.fp2;
				}
			}
		}
	...
	function mutePlayer(usernameOrFp2) {
		if (!usernameOrFp2 || usernameOrFp2.trim() === '') return;

		usernameOrFp2 = usernameOrFp2.trim();

		// Try to find fp2 from name if it's not already an fp2 value (fp2 is typically 64 chars)
		let fp2 = usernameOrFp2.length >= 64 ? usernameOrFp2 : findFp2FromName(usernameOrFp2);

		// Determine what to store: prefer fp2, fall back to name
		let muteIdentifier = fp2;
		let nameIdentifier = null;

		if (!fp2) {
			// If no fp2 found, use the name (remove prefixes like [ADMIN], [MOD], [SERVER])
			const cleanName = usernameOrFp2.replace(/^\[(?:ADMIN|MOD|SERVER)\]\s*/, '');
			muteIdentifier = cleanName;
		} else {
			// If fp2 is found, also get the name to check for existing name-based mutes
			const cleanName = usernameOrFp2.replace(/^\[(?:ADMIN|MOD|SERVER)\]\s*/, '');
			if (cleanName && cleanName.length < 64) {
				nameIdentifier = cleanName;
			}
		}

		if (!muteIdentifier) {
			chat.messages.push({ color: Color.fromHex('#ff3f3f'), name: '[SYSTEM]', message: `Could not find player "${usernameOrFp2}" in recent chat messages. Please mute them by clicking the mute icon next to their message.`, time: Date.now(), server: true, admin: false, mod: false, me: false, fp2: null });
			drawChat();
			return;
		}

		// If already muted by this identifier, return
		if (settings.mutedPlayers.includes(muteIdentifier)) return;

		// If muting by fp2 and name was previously muted, remove name entry to avoid muting wrong person
		if (fp2 && nameIdentifier && settings.mutedPlayers.includes(nameIdentifier)) {
			const nameIndex = settings.mutedPlayers.indexOf(nameIdentifier);
			if (nameIndex !== -1) {
				settings.mutedPlayers.splice(nameIndex, 1);
			}
		}

		settings.mutedPlayers.push(muteIdentifier);
		storeSettings();
		updateMutedPlayersList();
		chat.messages.push({ color: Color.fromHex('#3f3fc0'), name: '[SYSTEM]', message: `Player has been muted. You can unmute them in the Settings menu.`, time: Date.now(), server: true, admin: false, mod: false, me: false, fp2: null });
		drawChat();
	}

	function unmutePlayer(identifier) {
		if (!identifier || identifier.trim() === '') return;

		identifier = identifier.trim();

		const index = settings.mutedPlayers.indexOf(identifier);

		if (index !== -1) {
			settings.mutedPlayers.splice(index, 1);
			storeSettings();
			updateMutedPlayersList();
			chat.messages.push({ color: Color.fromHex('#3f3fc0'), name: '[SYSTEM]', message: `Player has been unmuted.`, time: Date.now(), server: true, admin: false, mod: false, me: false, fp2: null });
			drawChat();
		}
	}

	function findNameFromFp2(fp2) {
		if (!fp2) return null;

		// Search through recent chat messages to find name for this fp2
		for (let i = chat.messages.length - 1; i >= 0; i--) {
			const msg = chat.messages[i];
			if (msg.fp2 === fp2 && msg.name) {
				return msg.name;
			}
		}
	...
	function updateMutedPlayersList() {
		...
		settings.mutedPlayers.forEach(identifier => {
			...
			const canvas = document.createElement('canvas');
			canvas.width = 200;  // Adjust width for display
			canvas.height = 20;
			const ctx = canvas.getContext('2d');

			// Determine if identifier is fp2 (typically 64 chars) or name
			let displayText;
			if (identifier.length >= 64) {
				// It's likely an fp2, try to find name for it
				const name = findNameFromFp2(identifier);
				displayText = name || (identifier.length > 16 ? identifier.substring(0, 16) + '...' : identifier);
			} else {
				// It's a name, display it directly
				displayText = identifier;
			}
			drawTextOnto(canvas, ctx, displayText, 16, '#ffffff');
	...
	function isMessageMuted(message) {
		if (!message || !settings.mutedPlayers || settings.mutedPlayers.length === 0) return false;

		// IMPORTANT: If message has fp2, ONLY check fp2 to avoid muting wrong person with same name
		// Only check name if message has NO fp2 (to support legacy name-based mutes)
		if (message.fp2) {
			// Message has fp2: check fp2 ONLY (never check name to prevent muting different person with same name)
			// Use strict equality check to ensure exact match
			const fp2Str = String(message.fp2).trim();
			if (fp2Str && fp2Str.length > 0) {
				return settings.mutedPlayers.some(muted => String(muted).trim() === fp2Str);
			}
		}

		// Message has NO fp2: fall back to name-based muting (for backwards compatibility)
		if (message.name) {
			const nameStr = String(message.name).trim();
			const cleanName = nameStr.replace(/^\[(?:ADMIN|MOD|SERVER)\]\s*/, '').trim();

			// Check both the full name (with prefix) and clean name (without prefix)
			// Use strict equality check to ensure exact match
			return settings.mutedPlayers.some(muted => {
				const mutedStr = String(muted).trim();
				return mutedStr === nameStr || mutedStr === cleanName;
			});
		}
	...
	function drawChat() {
		...
		for (let i = endIndex - 1; i >= 0 && visibleMessages.length < visibleCount; i--) {
	...
	function cameraUpdate() {
		...
		camera.scale += (camera.target.scale - camera.scale) / settings.cameraAnimationSpeed;
	}
	...
	function keydown(event) {
		...
		// Ensure custom key bindings are tracked in pressed object
		...
		if (key === 'enter') {
			...
			// Split action
			if (key === settings.keySplit) {
				code = UINT8_CACHE[minionControlled ? 0x16 : 0x11];
				wsSend(code);
			}

			// Double split action
			if (key === settings.keyDoubleSplit) {
				code = UINT8_CACHE[minionControlled ? 0x16 : 0x11];
				wsSend(code);
				wsSend(code);
			}
			...
			// Minion control / Free-moving spectator camera
			if (key === settings.keyMinion) {
			...
			// Zoom in
			if (key === settings.keyZoomIn) {
				adjustZoom(0.4);
			}

			// Zoom out
			if (key === settings.keyZoomOut) {
				adjustZoom(-0.4);
			}
		}
	}
	...
	function keyup(event) {
		...
		// Mark key as released (works for both predefined and custom keys)
		if (key) pressed[key] = false;

		// Clear feed macro on key release
		if (key === settings.keyFeed || key === 'z') clearInterval(feedMacroIntervalID);
		// Clear split macro on key release
		if (key === settings.keyMultiSplit) clearInterval(splitMacroIntervalID);
	}
	...
	function announcePresence() {
		TC.postMessage({ type: 'check' });
		announcePresence2 = setTimeout(announcePresence, 1000);
	}
	...
	function init() {
		...
		if (typeof ChatFilter !== 'undefined') {
			...
			filters.initialize().then(function() {
				// Update preview after filters are initialized
				if (byId('previewName')) {
					byId('previewName').innerHTML = filterNicknameForClient(settings.nick);
				}
			}).catch(function(error) {
				console.error('Failed to initialize chat filter:', error);
			});
		}

		clearInterval(interval);
		interval = setInterval(() => {
			fetch('../fp2MuteList.txt').then(resp => resp.text()).then(data => {
		...
		const zoomButtons = ['zoomInBtn', 'zoomOutBtn'];
		const buttonIds = ['menuBtn', 'fullscreenBtn', 'splitBtn', 'ejectBtn', 'doubleSplitBtn', 'multiSplitBtn', ...zoomButtons];
		const DRAG_HOLD_DELAY = 2000;
		const DRAG_MOVE_THRESHOLD = 35; // allow slight finger movement before canceling hold
		const dragState = {
			activeButton: null,
			holdTimer: null,
			isDragging: false,
		...
		const startFeedAction = () => {
			clearInterval(feedMacroIntervalID);
			const code = UINT8_CACHE[minionControlled ? 0x17 : 0x15];
			const macroCooldown = settings.feedMacro ? 0 : 1000 / 7;
			feedMacroIntervalID = setInterval(() => wsSend(code), macroCooldown);
			wsSend(code);
		};

		const stopAutoAction = buttonId => {
			// Only stop actions when explicitly needed (e.g., on touch/click end)
			// Don't stop feed macro when drag border appears - let it continue
			if (buttonId === 'multiSplitBtn') {
				clearInterval(splitMacroIntervalID);
			}
		};

		const updateZoomButtonsVisibility = () => {
			const shouldShow = settings.showZoom && touched && !settings.disableTouchControls;
			zoomButtons.forEach((buttonId) => {
		...
		function loadButtonPositions() {
			...
			buttonIds.forEach(buttonId => {
				...
				const savedPos = settings.buttonPositions[buttonId];
				if (savedPos) {
					...
					if (savedPos.right !== undefined) {
						button.style.right = savedPos.right;
					...
					if (savedPos.bottom !== undefined) {
						button.style.bottom = savedPos.bottom;
		...
		function resetButtonPositions() {
			...
			// Reset all buttons to default CSS positions by removing inline styles
			buttonIds.forEach(buttonId => {
				const button = byId(buttonId);
				if (!button) return;

				// Remove all inline position styles to restore CSS defaults
				button.style.left = 'auto';
				button.style.right = 'auto';
				button.style.top = 'auto';
				button.style.bottom = 'auto';
				// Also remove inset property if it exists (safe to call even if property doesn't exist)
		...
		function saveButtonPosition(buttonId) {
			const button = byId(buttonId);
			if (!button) return;

			// Initialize buttonPositions if it doesn't exist
			if (!settings.buttonPositions) {
				settings.buttonPositions = {};
			}
			...
			// Save position values (prefer left/top when set, otherwise right/bottom)
			if (left !== 'auto' && left !== '' && parseFloat(left) >= 0) {
				pos.left = left;
				// Clear right if we're using left
				pos.right = undefined;
			} else if (right !== 'auto' && right !== '' && parseFloat(right) >= 0) {
				pos.right = right;
				// Clear left if we're using right
				pos.left = undefined;
			}

			if (top !== 'auto' && top !== '' && parseFloat(top) >= 0) {
				pos.top = top;
				// Clear bottom if we're using top
				pos.bottom = undefined;
			} else if (bottom !== 'auto' && bottom !== '' && parseFloat(bottom) >= 0) {
				pos.bottom = bottom;
				// Clear top if we're using bottom
				pos.top = undefined;
			}

			settings.buttonPositions[buttonId] = pos;
			storeSettings();
		}

		function startDragHold(buttonId, point) {
			// Don't start drag if buttons are locked
			if (settings.lockMobileButtons) {
				return;
			}
			...
			dragState.activeButton = buttonId;
			const button = byId(buttonId);
			if (!button) return;

			dragState.startX = point.pageX || point.clientX;
			dragState.startY = point.pageY || point.clientY;
			dragState.hasMoved = false;

			dragState.holdTimer = setTimeout(() => {
				// Don't stop feed macro here - let it continue even during drag
				// Only stop split macro if needed
				if (buttonId === 'multiSplitBtn') {
					clearInterval(splitMacroIntervalID);
				}
		...
		function cancelDragHold() {
			...
			if (dragState.activeButton) {
				const button = byId(dragState.activeButton);
				if (button) {
					button.classList.remove('draggable');
				}
			}
			dragState.activeButton = null;
			dragState.isDragging = false;
			dragState.hasMoved = false;
			dragState.pendingAction = null;
		}

		function handleButtonDrag(point) {
			if (!dragState.isDragging || !dragState.activeButton) return;

			const button = byId(dragState.activeButton);
			if (!button) return;

			const pageX = point.pageX || point.clientX;
			const pageY = point.pageY || point.clientY;
			const deltaX = pageX - dragState.startX;
			const deltaY = pageY - dragState.startY;
		...
		const handleTouchStart = (event) => {
			...
			// Check if touching a draggable button
			const isDraggableButton = buttonIds.indexOf(buttonId) !== -1;
			if (isDraggableButton) {
				const touch = event.touches[0];
				startDragHold(buttonId, { pageX: touch.pageX, pageY: touch.pageY });
			}
			...
			// Only handle movement if not starting a drag on a button
			if (!isDraggableButton && !settings.useJoystick) {
				handleTouchMove(event);

				touch.hide();
				touchCircle.show();
			} else if (!isDraggableButton) {
				touch.show();
				touchCircle.hide();
			}
		};
		...
		const handleTouchMove = event => {
			...
			if (dragState.activeButton && !dragState.isDragging) {
				const touch = event.touches[0];
				const moveDistance = Math.hypot(touch.pageX - dragState.startX, touch.pageY - dragState.startY);
			...
			if (dragState.isDragging && dragState.activeButton) {
				const touch = event.touches[0];
				handleButtonDrag({ pageX: touch.pageX, pageY: touch.pageY });
				event.preventDefault();
				return;
			}
		...
		const handleTouchEnd = (event) => {
			...
			if (dragState.isDragging && dragState.activeButton) {
				...
				// Stop feed macro when drag ends if it was the eject button
				if (dragState.activeButton === 'ejectBtn') {
					clearInterval(feedMacroIntervalID);
				}
				cancelDragHold();
				return;
			...
			// Execute pending action if user didn't drag
			if (dragState.pendingAction && !dragState.hasMoved && !dragState.isDragging) {
				dragState.pendingAction();
			}

			// Stop feed macro on touch end if it was the eject button
			if (event.target && event.target.id === 'ejectBtn') {
				clearInterval(feedMacroIntervalID);
			}
			...
			if (touched) {
				// Only clear split macro here, feed macro is handled above
				clearInterval(splitMacroIntervalID);
			}
		...
		window.addEventListener('mousedown', event => {
			...
			// Check if clicking a draggable button (check if button exists and is visible)
			if (buttonIds.indexOf(buttonId) !== -1) {
				const button = byId(buttonId);
				if (!button) return;
				const computedStyle = window.getComputedStyle(button);
				if (computedStyle.display !== 'none') {
					startDragHold(buttonId, event);
					// Store button actions to execute on mouseup if not dragging
					if (buttonId === 'splitBtn') {
						dragState.pendingAction = () => {
							wsSend(UINT8_CACHE[minionControlled ? 0x16 : 0x11]);
						};
					} else if (buttonId === 'doubleSplitBtn') {
						dragState.pendingAction = () => {
							const code = UINT8_CACHE[minionControlled ? 0x16 : 0x11];
							wsSend(code);
							wsSend(code);
						};
					} else if (buttonId === 'multiSplitBtn') {
						dragState.pendingAction = () => {
							clearInterval(splitMacroIntervalID);
							const code = UINT8_CACHE[minionControlled ? 0x16 : 0x11];
							const macroCooldown = settings.splitMacro ? 0 : 1000 / 7;
					...
					} else if (buttonId === 'ejectBtn') {
						startFeedAction();
						dragState.pendingAction = null;
					} else if (buttonId === 'zoomInBtn') {
						dragState.pendingAction = () => {
							adjustZoom(0.4);
						};
					} else if (buttonId === 'zoomOutBtn') {
						dragState.pendingAction = () => {
							adjustZoom(-0.4);
						};
					}
		...
		window.addEventListener('mouseup', event => {
			...
			// Stop feed macro on mouse up if it was the eject button
			if (event.target && event.target.id === 'ejectBtn') {
				clearInterval(feedMacroIntervalID);
			}
		...
		// Helper function to execute mouse button actions
		function executeMouseAction(action, isMouseDown) {
		...
		document.addEventListener('mousedown', event => {
			...
			switch (event.button) {
				case 0: // Left click
					if (settings.leftClick && settings.mouseLeftAction !== 'none') {
						executeMouseAction(settings.mouseLeftAction, true);
					}
					break;
				case 1: // Middle click
					if (settings.middleClick && settings.mouseMiddleAction !== 'none') {
						executeMouseAction(settings.mouseMiddleAction, true);
					}
					break;
			}
		});
		...
		document.addEventListener('mouseup', event => {
			...
			switch (event.button) {
				case 0: // Left click
					if (settings.leftClick && settings.mouseLeftAction !== 'none') {
						executeMouseAction(settings.mouseLeftAction, false);
					}
					break;
				case 1: // Middle click
					if (settings.middleClick && settings.mouseMiddleAction !== 'none') {
						executeMouseAction(settings.mouseMiddleAction, false);
					}
					break;
			}
		});
		...
		mainCanvas.addEventListener('click', event => {
			...
			for (let i = 0; i < chat.muteIconAreas.length; i++) {
				...
				if (scaledMouseX >= areaX && scaledMouseX <= areaX + area.width && scaledMouseY >= areaY && scaledMouseY <= areaY + area.height) {
					...
					// Determine identifier: prefer fp2, fall back to name
					let identifier = fp2;
					if (!identifier && username) {
						// Remove prefixes like [ADMIN], [MOD], [SERVER] for consistency
						identifier = username.replace(/^\[(?:ADMIN|MOD|SERVER)\]\s*/, '');
					}

					if (identifier) {
						if (settings.mutedPlayers.includes(identifier)) {
							unmutePlayer(identifier);
					} else {
							mutePlayer(identifier);
						}
					}
		...
		setInterval(() => {
			// Only send mouse movement if player has cells (normal gameplay) or if minionControlled is true (free camera)
			if (cells.mine.length > 0 || minionControlled) {
		...
		function updateKeyBindingButton(action, key) {
			const btn = byId(action + '-btn');
			if (btn) {
				btn.textContent = getKeyDisplayName(key);
			}
		}

		function updateGameplayInstructions() {
			const feedDisplay = byId('key-feed-display');
			const splitDisplay = byId('key-split-display');
			const doubleSplitDisplay = byId('key-doublesplit-display');
			const multiSplitDisplay = byId('key-multisplit-display');
			const zoomInDisplay = byId('key-zoomin-display');
			const zoomOutDisplay = byId('key-zoomout-display');

			if (feedDisplay) feedDisplay.textContent = getKeyDisplayName(settings.keyFeed);
			if (splitDisplay) splitDisplay.textContent = getKeyDisplayName(settings.keySplit);
			if (doubleSplitDisplay) doubleSplitDisplay.textContent = getKeyDisplayName(settings.keyDoubleSplit);
			if (multiSplitDisplay) multiSplitDisplay.textContent = getKeyDisplayName(settings.keyMultiSplit);
			if (zoomInDisplay) zoomInDisplay.textContent = getKeyDisplayName(settings.keyZoomIn);
			if (zoomOutDisplay) zoomOutDisplay.textContent = getKeyDisplayName(settings.keyZoomOut);
		}

		// Initialize button labels from settings
		updateKeyBindingButton('keyFeed', settings.keyFeed);
		updateKeyBindingButton('keySplit', settings.keySplit);
		updateKeyBindingButton('keyDoubleSplit', settings.keyDoubleSplit);
		updateKeyBindingButton('keyMultiSplit', settings.keyMultiSplit);
		updateKeyBindingButton('keyZoomIn', settings.keyZoomIn);
		updateKeyBindingButton('keyZoomOut', settings.keyZoomOut);
		updateKeyBindingButton('keyMinion', settings.keyMinion);

		// Initialize gameplay instructions
		updateGameplayInstructions();

		// Mouse button binding setup
		const mouseLeftActionSelect = byId('mouseLeftAction');
		const mouseMiddleActionSelect = byId('mouseMiddleAction');
		const mouseRightActionSelect = byId('mouseRightAction');

		if (mouseLeftActionSelect) {
			mouseLeftActionSelect.addEventListener('change', () => {
				settings.mouseLeftAction = mouseLeftActionSelect.value;
				storeSettings();
			});
		}

		if (mouseMiddleActionSelect) {
			mouseMiddleActionSelect.addEventListener('change', () => {
				settings.mouseMiddleAction = mouseMiddleActionSelect.value;
				storeSettings();
			});
		}

		if (mouseRightActionSelect) {
			mouseRightActionSelect.addEventListener('change', () => {
				settings.mouseRightAction = mouseRightActionSelect.value;
				storeSettings();
			});
		}

		// Reset mouse bindings button
		const resetMouseBindingsBtn = byId('reset-mousebindings-btn');
		if (resetMouseBindingsBtn) {
			resetMouseBindingsBtn.addEventListener('click', () => {
				const defaults = {
					mouseLeftAction: 'feed',
					mouseMiddleAction: 'multiSplit',
					mouseRightAction: 'doubleSplit'
				...
				settings.mouseLeftAction = defaults.mouseLeftAction;
				settings.mouseMiddleAction = defaults.mouseMiddleAction;
				settings.mouseRightAction = defaults.mouseRightAction;

				storeSettings();

				if (mouseLeftActionSelect) mouseLeftActionSelect.value = defaults.mouseLeftAction;
				if (mouseMiddleActionSelect) mouseMiddleActionSelect.value = defaults.mouseMiddleAction;
				if (mouseRightActionSelect) mouseRightActionSelect.value = defaults.mouseRightAction;
			});
		}

		// Reset key bindings button
		const resetKeyBindingsBtn = byId('reset-keybindings-btn');
		if (resetKeyBindingsBtn) {
			resetKeyBindingsBtn.addEventListener('click', () => {
				...
				settings.keyFeed = defaults.keyFeed;
				settings.keySplit = defaults.keySplit;
				settings.keyDoubleSplit = defaults.keyDoubleSplit;
				settings.keyMultiSplit = defaults.keyMultiSplit;
				settings.keyZoomIn = defaults.keyZoomIn;
				settings.keyZoomOut = defaults.keyZoomOut;
				settings.keyMinion = defaults.keyMinion;

				storeSettings();

				updateKeyBindingButton('keyFeed', defaults.keyFeed);
				updateKeyBindingButton('keySplit', defaults.keySplit);
				updateKeyBindingButton('keyDoubleSplit', defaults.keyDoubleSplit);
				updateKeyBindingButton('keyMultiSplit', defaults.keyMultiSplit);
				updateKeyBindingButton('keyZoomIn', defaults.keyZoomIn);
				updateKeyBindingButton('keyZoomOut', defaults.keyZoomOut);
				updateKeyBindingButton('keyMinion', defaults.keyMinion);
		...
		keyBindingButtons.forEach((btn) => {
			btn.addEventListener('click', () => {
				if (currentlyBinding) {
					currentlyBinding.classList.remove('btn-warning');
					currentlyBinding.classList.add('btn-info');
				}
		...
		window.addEventListener('keydown', (event) => {
			...
			const action = currentlyBinding.getAttribute('data-action');
			let key = processKey(event);

			// Prevent binding special keys
			if (key === 'enter' || key === 'escape' || key === 'tab') {
				currentlyBinding.classList.remove('btn-warning');
				currentlyBinding.classList.add('btn-info');
				updateKeyBindingButton(action, settings[action]);
			...
			updateKeyBindingButton(action, key);
