$(function() {
	...
	var isSpectating = urlParams.has('spectate');
	var initialNick = urlParams.get('nick') || '';
	var initialSkin = urlParams.get('skin') || '';

	// Set initial values from URL or localStorage
	$nick.val(initialNick || getStoredValue(storageKeys.nick, legacyKeys.nick));
	$skin.val(initialSkin || getStoredValue(storageKeys.skin, legacyKeys.skin));
	...
	// Auto-start based on URL parameters
	if (isSpectating) {
		spectate();
	} else if (initialNick) {
		setNick('<' + getPlayableSkin() + '|' + (playerIdent ? playerIdent : '') + '>' + sanitizeNick(initialNick));
	}

	// Handle exit back to lobby
	...
	// Override ESC key to exit to lobby instead of showing overlays
	var originalOnKeyDown = wHandle.onkeydown;
	wHandle.onkeydown = function(event) {
		...
		switch (event.keyCode) {
			case 27: // ESC key
				...
				// Exit to lobby instead of showing overlays
				if (ws) {
					ws.onclose = null;
					try {
						ws.close();
	...
	// Handle death/exit - redirect to lobby when player cells become empty
	var originalShowOverlays = showOverlays;
	showOverlays = function(arg) {
		// When player dies (no cells left), redirect to lobby
		if (!arg && playerCells.length === 0) {
			if (ws) {
				ws.onclose = null;
				try {
					ws.close();
		...
		originalShowOverlays(arg);
	};

	// Load ban list and connect
	$.get('fpBanList.txt').success(function(data) {
		...
		FP.then(function(fp) { return fp.get(); }).then(function(result) {
			...
			if (ban) {
				$('#chat_textbox').hide();
				$('#overlays').hide();
				$('#connecting div').html('<h3 style="text-align: center">You are banned</h3><hr class="top" /><p style="text-align: center">You are banned from Blobz because of repeated rule violations.</p><h1 style="text-align: center;">Your unban code is<br /><br />' + btoa(playerIdent).replace(/(.{10})/g, "$1<br />") + '</h1>');
				$('#connecting').show();
			} else {
				$('#overlays').show();
				connect(socketUrl);
				...
				interval = setInterval(function() {
					FP.then(function(fp) { return fp.get(); }).then(function(result) {
						...
						if (liveBan) {
							$('#chat_textbox').hide();
							$('#overlays').hide();
							$('#connecting div').html('<h3 style="text-align: center">You are banned</h3><hr class="top" /><p style="text-align: center">You are banned from Blobz because of repeated rule violations.</p><h1 style="text-align: center;">Your unban code is<br /><br />' + btoa(liveIdent).replace(/(.{10})/g, "$1<br />") + '</h1>');
							$('#connecting').show();
						}
					});
				}, 10000);
